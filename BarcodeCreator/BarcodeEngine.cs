		...
		public List<BarcodeItem> Generate(string barcode)
		{
			switch (CodeType)
			{
				...
				case CodeTypes.Code128C:
					if (barcode.Any(x => x is > '9' or < '0')) return new();
				...
				case CodeTypes.Code39:
					...
					foreach (var item in barcode)
					{
						...
						for (var i = 0; i < codeItem.Length; i++)
						{
							...
							{
								Color = (codeItem[i] is 'W' or 'N') ? Brushes.Black : Brushes.Transparent,
								Width = ((codeItem[i] is 'n' or 'N') ? 1 : Code39WideRate) * BarWidth
							});
						}
					}
					return result;
