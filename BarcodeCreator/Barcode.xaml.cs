namespace BarcodeCreator
{
	...
	public partial class Barcode : UserControl, INotifyPropertyChanged
	{
		...
		private static readonly DependencyProperty CodeProp = DependencyProperty.Register(nameof(Code), typeof(string), typeof(Barcode), new(CodeMetadata));
		private static readonly DependencyProperty TypeProp = DependencyProperty.Register(nameof(CodeType), typeof(string), typeof(Barcode), new($"{CodeTypes.Code39}", CodeTypeMetadata));
		private static readonly DependencyProperty WideRateProp = DependencyProperty.Register(nameof(Code39WideRate), typeof(double), typeof(Barcode), new(1d, WideRateMetadata));
		private static readonly DependencyProperty VisibilityProp = DependencyProperty.Register(nameof(TextVisibility), typeof(Visibility), typeof(Barcode), new(Visibility.Visible, TextVisibilityMetadata));
		private static readonly DependencyProperty WidthProp = DependencyProperty.Register(nameof(BarWidth), typeof(double), typeof(Barcode), new(1.5, BarWidthMetadata));

		private readonly BarcodeEngine barcodeEngine;
		private readonly ObservableCollection<BarcodeItem> barCodeItems = new()
		{
		...
		private static void CodeTypeMetadata(object sender, DependencyPropertyChangedEventArgs e)
		{
			...
			bc.barcodeEngine.CodeType = Enum.Parse<CodeTypes>((string)e.NewValue);
			bc.barCodeItems.Clear();
